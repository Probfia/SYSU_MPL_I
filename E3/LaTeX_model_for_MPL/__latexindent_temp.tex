...
这是由于等离子体中含有电子，电子在极板电压的加速下获得动能，运动的电子撞击原子，损失动能，这个过程可以用两个耦合的旋量场描述
\beq
...
四极电场由两组相对的电极产生(分别设为$x$方向，$y$方向)。先来看它们对时间的依赖关系，两个方向上的电场都是交变电场，一般是以正弦形式变化。而至于它们对空间坐标的依赖关系，就是电场的大小与坐标值成正比（$x$方向电场正比于$x $的值，$y$方向的电场正比于$y$的值）。于是电势是一个随时间变化的马鞍面形式
\beq
...
\\
2.\textbf{验证帕邢定律}\par
调整极板间距为$5\unit{cm}$，调节真空气压$V$，调节电压$p$使得剩余气体刚好被击穿，测得$p-V$曲线如
%\cpicn{0.4}{p3}{极板间距为$5\unit{cm}$时的$p-V$曲线}
\par
从拟合的曲线可以得到帕邢定律
\beq
V_{S}=\frac{B P d}{\ln \left(\frac{A P d}{\ln (1+1 / \gamma)}\right)}
\eeq
中的各个参数
\bea
B &= \\
...
%\cpicn{0.4}{p4}{极板间距为$4\unit{cm}$时的$p-V$曲线}
\par
从拟合的曲线可以得到帕邢定律
\beq
V_{S}=\frac{B P d}{\ln \left(\frac{A P d}{\ln (1+1 / \gamma)}\right)}
...
\gamma &= \\
\eea
可以看到，两次测量对应的参数值相仿。合成测量结果为
\beq
B =\frac{\frac{B_1}{\sigma_{B_1}^2} + \frac{B_2}{\sigma_{B_2}^2}}{ \frac{1}{\sigma_{B_1}^2} + \frac{1}{\sigma_{B_2}^2}}= 
\eeq
\beq
A =\frac{\frac{A_1}{\sigma_{A_1}^2} + \frac{A_2}{\sigma_{A_2}^2}}{ \frac{1}{\sigma_{A_1}^2} + \frac{1}{\sigma_{A_2}^2}}= 
\eeq
\beq
\sigma_B = \sqrt{\frac{1}{\sigma_{B_1}^2} + \frac{1}{\sigma_{B_2}^2}} =
\eeq
\beq
\sigma_A = \sqrt{\frac{1}{\sigma_{A_1}^2} + \frac{1}{\sigma_{A_2}^2}} =
\eeq
于是综合结果表示为
\bea
A &= \\
...
保持真空度为$\sim 1\times 10^{-2} \unit{Pa}$，改变分辨率和加速电压，得到几组扫描结果如
...
\newpage
\cpicn{0.28}{9.6e-3_air_resolu_60_emi_0.1.JPG}{真空度为$0.96\times 10^{-2} \unit{Pa}$改变分辨率为60，发射系数为0.1下的扫描结果}
\cpicn{0.28}{9.6e-3_air_resolu_60_emi_0.2}{真空度为$0.96\times 10^{-2} \unit{Pa}$改变分辨率为60，发射系数为0.1下的扫描结果}
\cpicn{0.28}{9.6e-3_air_resolu_60_eVol_40}{真空度为$0.96\times 10^{-2} \unit{Pa}$改变分辨率为60，电压为40下的扫描结果}
\cpicn{0.28}{9.7e-3_air_resolu_20}{真空度为$0.97\times 10^{-2} \unit{Pa}$改变分辨率为20下的扫描结果}
\cpicn{0.28}{9.7e-3_air_resolu_40}{真空度为$0.97\times 10^{-2} \unit{Pa}$改变分辨率为40下的扫描结果}
\cpicn{0.28}{9.7e-3_air_resolu_50}{真空度为$0.97\times 10^{-2} \unit{Pa}$改变分辨率为50下的扫描结果}
\cpicn{0.28}{9.8e-3_air_resolu_20}{真空度为$0.98\times 10^{-2} \unit{Pa}$改变分辨率为20下的扫描结果}
\cpicn{0.28}{96.e-3_air_resolu_60_eVol_80}{真空度为$0.97\times 10^{-2} \unit{Pa}$改变分辨率为60，电压为80下的扫描结果}
\cpicn{0.28}{96.e-3_air_resolu_60}{真空度为$0.96\times 10^{-2} \unit{Pa}$改变分辨率为60下的扫描结果}
