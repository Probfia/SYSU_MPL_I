...
\textbf{【实验前思考题】}
\begin{enumerate}
 \item 请写出拍频信号的数学表达式，并分别用时域和频域图表述出来。
 \par
 ...
 \item 噪声电压信号是个随机值，原则上，只要时间足够长，其平均值为零。如果待测信号是个恒定值，且环境噪声也是稳定的，则可以通过大量采样平均来获得待测信号值。 然而，实际测量中，信号不是常量，而是（相对）慢变量，而（环境）噪声也不稳定， 这样长时间大量采样平均的方法就不奏效，锁相放大器是个更好的解决方案。为更好的使用锁相放大器，请回答以下问题：
 \begin{enumerate}
 \item 如何用锁相放大器检测到待测的直流信号或慢变信号？
 \par 由于$s(t)$为慢变量，因此可以用锁相放大器按\cref{measure}测量锁相放大器的输出信号随时间的变化，输出信号$u_o(t) \propto s(t) + n_l (t)$，$n_l$为低频噪声，功率谱主要在低频部分。当然，根据输入信号的特征频率大小，进行低通滤波时的截断频率$\omega_c$也需要随着调整大小。
  \item 如用斩波器调制直流信号（如光强），被斩制后的信号仍然包含有直流分量（即平均值不为零），但该直流分量随交流信号输入锁相放大器后不会被锁相放大器检测，请从数学推导上说明。
 \par
 \item 用复数的取代三角函数，将锁相放大器的工作过程描述一遍；
\par
...
\item 用一条 BNC-BNC 信号线连接 OE1022 前面板 SINE OUT 输出接口和 SIGNAL IN 的 A/I
 ...
 \item 如\cref{d115}，按下软键1以选中Sensitivity功能，通过旋转旋钮调节Sensitivity 值，使测量信号值尽量满偏但不超量程，到$100\unit{mV}$左右即可。此时我们测出信号发生器发生的正弦波幅值和相位$A=$\uline{\hspace{2cm}}，相位$\theta = $\uline{\hspace{2cm}}，参考结果见\cref{d115}。
